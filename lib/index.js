...
function normalize(options){
  ...
  options.stripHtml = (typeof options.stripHtml === 'undefined') ? false : options.stripHtml;
  return options;
}
...
function replace(pattern, data, options){
  ...
  for (var i = 0, key; key = keys[i++];) {
    ...
    if (val instanceof Date) {
      ...
      ret[key] = slug((options.stripHtml ? sanitizeHtml(val.toString(), {allowedTags: [], allowedAttributes:[]}): val.toString()));
    }
  }
